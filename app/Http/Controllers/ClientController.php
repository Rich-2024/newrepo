...
use App\Mail\LoanCreationNotification;
...
public function create(Request $request)
{
    ...
    foreach ($validated['clients'] as $clientData) {
        ...
        if ($existingClient) {
            $errors[] = "Client with contact {$clientData['contact']} already has an active loan: {$existingClient->name}.";
            continue;
        }
      ...
      Mail::to(Auth::user()->email)->send(new LoanCreationNotification(
    $adminName,                          // Admin's name (issuer)
    $clientData['loan_date'],           // Loan issue date
    $clientData['name'],                // Client name
    $clientData['contact'],             // Client contact
    $amount,                            // Loan amount
    $rate,                              // Interest rate
    round($totalToPay, 2),              // Total amount to be repaid
    $loanEndDate->toDateString()        // Loan end date
));
