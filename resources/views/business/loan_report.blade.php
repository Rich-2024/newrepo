...
<div class="w-full max-w-7xl mx-auto bg-white p-8 rounded-lg shadow-lg mt-8">
 ...
 <section class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-md mb-12 border border-gray-200">
   ...
   @php
    ...
    $receivables = \App\Models\SettledLoan::where('user_id', $userId)->sum('balance_left');

    $receivabledPaid = \App\Models\SettledRepayment::whereHas('settledLoan', function ($query) use ($userId) {
        $query->where('user_id', $userId);
    })->sum('amount');
@endphp
